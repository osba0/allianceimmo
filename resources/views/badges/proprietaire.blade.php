...
<head>
  ...
  <title>Badge Propriétaire</title>
...
<div class="container">
  ...
  <div class="text-center my-3 header-logo">
    ...
    <div class="text-center mt-3">
         <a href="{{ route('badgeProprietaire.logout', ['proprietaire_id' => $proprietaire->proprio_id]) }}" onclick="return confirm('Êtes-vous sûr de vouloir vous déconnecter ?')" class="btn btn-outline-danger btn-sm">
        🔓 Se déconnecter
      </a>
    </div>
  </div>
  ...
  <!-- Infos -->
  <div class="card p-3">
    <h4 class="text-center mb-4">Bonjour {{ $proprietaire->proprio_prenom }} {{ $proprietaire->proprio_nom }}</h4>

    <h5 class="text-primary text-center">📋 Vos Versements 6 derniers mois:</h5>
      ...
      <table class="table table-striped table-bordered">
        ...
        <tbody>
          @foreach ($versements as $versement)
            ...
            <tr>
              <td>{{ \Carbon\Carbon::parse($versement->paiement_mois_location)->format('m/Y') }}
              </td>
              <td>{{ number_format($versement->total_paye, 0, ',', ' ') }} FCFA</td>
              <td>
                @if ($versement->statut == 'payé')
                  <span class="badge">✅ Payé</span>
                @elseif ($versement->statut == 'partiel')
